...
const AdminAddInvoice = () => {
  ...
  //! Add invoice to db
  async function handleAddInvoice(status) {
    try {
  ...
  useEffect(() => {
    ...
    const isAnyFieldFilled = Boolean(
      ...
      (lastRow?.hsnCode && String(lastRow.hsnCode).trim() !== "") ||
      (lastRow?.cgst && String(lastRow.cgst).trim() !== "") ||
      (lastRow?.sgst && String(lastRow.sgst).trim() !== "")
    );
    ...
    <AdminLayout>
      ...
      {previewMode ? (
        ...
        <>
          ...
          <button
            onClick={() => handleAddInvoice("unpaid")}
            className="mt-3 mx-2 b-btn py-2 mt-4"
          >
            Save Invoice as Unpaid
          </button>
          <button
            onClick={() => handleAddInvoice("paid")}
            className="mt-3 mx-2 b-btn py-2 mt-4"
          >
...
export default AdminAddInvoice;
